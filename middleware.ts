...
export async function middleware(request: NextRequest) {
  ...
  if (!user && request.nextUrl.pathname.startsWith("/read")) {
    return NextResponse.redirect(new URL("/", request.url));
  }
  ...
  if (!user && request.nextUrl.pathname.startsWith("/challenges")) {
    return NextResponse.redirect(new URL("/", request.url));
  }
...
export const config = {
  matcher: ["/dashboard/:path*", "/read/:path*", "/plans/:path*", "/notes/:path*", "/chat/:path*", "/challenges/:path*"],
};
